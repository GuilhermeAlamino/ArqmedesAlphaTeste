...
if (isset($_SESSION['edit_category_id'])) {
  $editCategoryId = $_SESSION['edit_category_id'];


  $controller = new AddCategoryController();
  $categoriesShow = $controller->show($editCategoryId);
  // var_dump($categoriesShow);
  // die();
} else {
...
<!-- Main Content -->
<main class="content">
  <h1 class="title new-item">Update Category</h1>

  <form action="?view=updateCategoriesController" method="POST">
    <input type="hidden" name="id" id="id" value="<?php echo htmlspecialchars($categoriesShow['id']); ?>">
    <div class="input-field">
      <label for="category-name" class="label">Category Name</label>
      <input type="text" id="category-name" value="<?php echo htmlspecialchars($categoriesShow['name']); ?>" name="category-name" class="input-text" />

    </div>
    <div class="input-field">
      <label for="category-code" class="label">Category Code</label>
      <input type="text" id="category-code" value="<?php echo htmlspecialchars($categoriesShow['code']); ?>" name="category-code" class="input-text" />

    </div>
    <div class="actions-form">
      <a href="?view=categories" class="action back">Back</a>
